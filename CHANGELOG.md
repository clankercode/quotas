...
- **TUI footer always shows the current package version** (`vX.Y.Z`) on the bottom-right path. When a newer release is available the existing cyan `update X.Y.Z` link still appears to the right of that label (only the update text is clickable).
