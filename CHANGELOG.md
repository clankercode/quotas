...
- **Relicensed to Unlicense OR CC0-1.0** (public-domain dedication). `Cargo.toml` now matches the existing `LICENSE-UNLICENSE` / `LICENSE-CC0` files and README; crates.io metadata no longer claims MIT OR Apache-2.0.
...
- **TUI footer always shows the current package version** (`vX.Y.Z`) on the bottom-right path. When a newer release is available the cyan `update X.Y.Z` text is a clickable GitHub release link to the right of that label (only the update text is clickable).
- **Codex weekly windows labeled `7d`** (not `168h`) when the period is 604800s, at parse and draw time, with cache rehydrate so older cached entries pick up the label.
...
- **Dependency updates:** `reqwest` 0.12 → 0.13 (`rustls-tls` feature renamed to `rustls`; enable explicit `form` for OAuth token refresh), `rusqlite` 0.37 → 0.40 (bundled SQLite via `libsqlite3-sys` 0.38), `toml` 0.9 → 1.1. Declared MSRV raised to **1.95** (`libsqlite3-sys` 0.38 uses `std::cfg_select`, stable since 1.95; toml 1.1 alone would only need 1.85). Note: `generic-array` is only a transitive dep (via `digest`/`sha2` from the ratatui/termwiz stack) and remains on 0.14 — there is no direct way to pull 1.x until those crates migrate.
